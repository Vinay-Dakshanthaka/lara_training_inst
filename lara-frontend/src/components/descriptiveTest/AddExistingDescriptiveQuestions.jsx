...
const AddExistingDescriptiveQuestions = () => {
        ...
        try {
           ...
           console.log(topic_id,"-----------------------------topic id");
            const response = await axios.get(
                `${baseURL}/api/weekly-test/getQuestionsByTopicId/${topicId}`,               
            ); 
       console.log(response.data,"------------------topic avalible questions")
    ...
    const handleAddQuestions = async () => {
        try {
            ...
            const response = await axios.post(
                `${baseURL}/api/weekly-test/assignQuestionsToPlacementTest`,
                requestBody,
               
            );
             console.log(response.data,"----------------------assignQuestionsToPlacementTest")
                            ...
                            <>
                                <ul>
                                    {paginatedQuestions.map((question) => (
                                        <li key={question.wt_question_id} style={{ listStyle: 'none' }} className='my-2 border p-2'>
                                            <Form.Check
                                                type="checkbox"
                                                id={`question-${question.wt_question_id}`}
                                                label={question.wt_question_description}
                                                checked={selectedQuestions.includes(question.wt_question_id)}
                                                onChange={() => handleCheckboxChange(question.wt_question_id)}
                                            />
                                        </li>
                                    ))}
                                </ul>
...
export default AddExistingDescriptiveQuestions;
