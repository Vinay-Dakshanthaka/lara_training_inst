...
import DescriptiveTestDetails from '../descriptiveTest/DescriptiveTestDetails';

const AddDescriptiveTestQuestion = () => {
    const { placement_test_id } = useParams(); // Get wt_id from URL params
    ...
    useEffect(() => {
        const fetchWeeklyTestDetails = async () => {
            ...
            try {
                ...
                const response = await axios.get(`${baseURL}/api/weekly-test/getDescriptiveTestById/${placement_test_id}`, config);
                const test = response.data.test;
                console.log("test : ", test)
                // Set available topics from the fetched test
                const topics = test.TestTopics.map(topic => ({
                    topic_id: topic.PlacementTestTopic.topic_id,
                    name: topic.PlacementTestTopic.name
                }));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.post(`${baseURL}/api/weekly-test/savePlacementTestQuestionHandler`, data, config);
...
export default AddDescriptiveTestQuestion;
